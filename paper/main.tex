% arXiv submission: primary category cs.LG, secondary cs.CL. License: CC BY 4.0.
...
\emph{not} beat the Transformer). At inference the carried state is constant in $n$ ($0.56$\,MB / $147{,}456$ floats), analytically
$28$--$455\times$ fewer floats than a growing KV-cache and a measured constant $17.9$\,MB peak
($2.8$--$31\times$ less than the Transformer); a measured $O(1)$-latency crossover appears at
$n{\geq}32\text{k}$ ($2.4$--$2.8\times$ faster at $n{=}65\text{k}$), while below $\sim$16k it is
$\sim$1.3--1.6$\times$ \emph{slower}. A causal ablation shows the gain is the quadratic
...
the figure-generation code are committed at \url{https://github.com/nazmiefearmutcu/Prizma} (see
\texttt{results/} and \texttt{paper/figures/}). Each figure in this paper traces to a named JSON
under \texttt{results/}; the per-figure provenance map is
\texttt{paper/\allowbreak figures/\allowbreak figdata/\allowbreak PROVENANCE.md}.
...
\midrule
MQAR ($D{=}128$) & PASS &
Parity at 860K matched params (\quadtwo{} median 0.998, seeds $[0.996,0.998,0.999]$ vs.\ TF 0.9995);
solves at 130K (3/3, seeds $[0.935,0.997,0.999]$) where the matched TF fails (0/3) and the smallest
grid TF solver needs $\geq$461K $\Rightarrow$ $\geq$3.5$\times$ param-efficiency (coarse grid). \\
\addlinespace[2pt]
Induction & PASS &
...
$\quadtwo$ (3/3, median 0.997) $\gg$ rand\_linear (0/3, $\approx$0.59) $\approx$ none (0/3,
$\approx$0.52) $\gg$ TF (0/3, 0.016). The gain is the monomials, not ``a bigger RNN.'' \\
\addlinespace[2pt]
