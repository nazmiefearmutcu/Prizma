...
Trains the matched tiny TF, Prizma-none, and Prizma-quad2 on MixedMQAR(pairs 1..target), evaluates at
the FIXED target D, seed 0, punchy short-warmup schedule. If the TF now solves -> the path is found
(fair, applies to both); and we read off whether the zero-param quad2 lever holds Prizma at parity.
...
def main():
    ...
    for name, make in [("Transformer", tf), ("Prizma-none", ps), ("Prizma-quad2", psq)]:
        m = make()
        p = param_count(m)
