...
def b1_verdict():
    ...
    lines = ["### B1 — MQAR (decisive gate)", "", "| rung | PRISM-Seq | Transformer | Δ | verdict |", "|---|---|---|---|---|"]
    ok = True
    for rung in ["rung1", "rung2", "rung3"]:
        if rung not in r:
            continue
        ps, tf = r[rung]["PRISM-Seq"], r[rung]["Transformer"]
...
def b1b_verdict(d_star=None):
    ...
    lines = ["### B1b — MQAR capacity (recall vs #bindings D)", "",
             "| D | Transformer | PRISM d_h=32 | PRISM d_h=64 |", "|---|---|---|---|"]
    for D in Ds:
        tf = r["transformer"].get(str(D)) or r["transformer"].get(D)
        p32 = r["prism_by_dh"].get("32", {}).get(str(D)) or r["prism_by_dh"].get("32", {}).get(D)
        p64 = r["prism_by_dh"].get("64", {}).get(str(D)) or r["prism_by_dh"].get("64", {}).get(D)
        lines.append(f"| {D} | {_fmt(tf)} | {_fmt(p32)} | {_fmt(p64)} |")
    lines.append("\n*(D\\* = largest D where PRISM-Seq stays within 0.03 of the Transformer.)*")
    return "\n".join(lines), True
...
def b3_verdict():
    ...
    lines = ["### B3 — Selective copy", "", "| variant | PRISM-Seq | Transformer | PRISM-noGate |", "|---|---|---|---|"]
    for var in ["selective", "fixed"]:
        if var not in r:
            continue
        lines.append(f"| {var} | {_fmt(r[var].get('PRISM-Seq'))} | {_fmt(r[var].get('Transformer'))} "
                     f"| {_fmt(r[var].get('PRISM-noGate'))} |")
    sel = r.get("selective", {})
    ps = sel.get("PRISM-Seq", {}).get("mean", 0)
    ng = sel.get("PRISM-noGate", {}).get("mean", 1)
...
def b5_verdict():
    ...
    lines = ["### B5 — Inference cost (the structural advantage)", "",
             "| n | TF KV-cache (s) | PRISM step (s) | TF state (floats) | PRISM state (floats) |",
             "|---|---|---|---|---|"]
    tl, pl = r["transformer_kvcache_s"], r["prism_step_s"]
    tm, pm = r["transformer_state_floats"], r["prism_state_floats"]
    for n in ns:
        sn = str(n)
