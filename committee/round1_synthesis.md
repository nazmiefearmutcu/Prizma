# Prizma-Seq Committee — Round 1 Synthesis (durable record)
...
provisional "Prizma wins rung2/3" headline. And the pre-registered recall ceiling is real:
...
1. **Undertraining.** Fixed 6–8k step budgets sat near Prizma's fast convergence; the Transformer was
...
2. **LR grid asymmetry.** `{1e-3,2e-3}`: 2e-3 collapses the TF to 0.149 on rung1 while Prizma is robust
   at both → "best-of-2" gave Prizma a 2-shot draw, the TF a 1-shot. → **Wider shared grid
...
Both models are bimodal/LR-fragile on MQAR at this scale (`stability_test.txt`: TF 2/5, Prizma 5/5 on
...
- Transformer = n-growing lossless KV cache → unbounded recall; D=128 trivial. The structural trade:
  Prizma compresses D bindings into a fixed `d_h²` lossy codebook in exchange for O(1)/step inference.
...
1. **Param match** every head-to-head; any paid arm (#2/#3/#4) grows the Transformer in lockstep — never 130K-Prizma vs 102K-TF.
2. **Measured FLOP + throughput ledger** (Prizma carries an extra window head + conv at equal params; disclose direction+magnitude of any >10% gap; Prizma is ~3–5× slower wall-clock — disclose).
...
4. **O(1) equality guard**: streaming `step()` == parallel `forward()` to <1e-4 for every mechanism BEFORE any accuracy number (hard blocker; the `prizma_seq.py` __main__ assert).
...
8. **Both sides of the contrast**: verify the matched TF actually reaches near-ceiling at D=128 under the fair protocol before claiming "Prizma degrades, TF doesn't".
...
- `seq/prizma_seq.py`: `feat_map='quad2'` (config, fixed monomial buffers, `_phi`, rectangular state in forward/step/init_state) — **done, verified**: feat_map=quad2 → d_φ=128, params 102,728 (unchanged), step==forward 3.6e-7.
