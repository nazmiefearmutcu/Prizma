"""
PRISM validation suite -- the falsifiability gate.

Runs the headline comparison + the honesty stress-tests over multiple seeds and writes
results/results.json. Nothing here uses task labels or task boundaries for PRISM.

Experiments:
  E1  main comparison (structured-permuted, cleanly distinguishable domains), >=10 seeds, 95% CI
  E2  SEPARABILITY sweep: add iid noise -> domains blur -> where does PRISM break? (the boundary)
  E3  CAPACITY sweep: n_experts vs n_domains (under-capacity must hurt)
  E4  LOCALITY variant: PRISM with random feedback (DFA, no W^T at all) -- open-problem P2
  E5  IMPOSSIBLE-REGIME control: rotating-checkerboard (same input, different label) -- PRISM
...
def run_prism(tasks, d, ncls, init_seed, epochs, n_experts, h, consolidate=True,
              feedback="random", z_novel=5.0):
    p = PRISM(d=d, h=h, K=ncls, n_experts=n_experts, seed=init_seed,
...
def run_oracle_multihead(tasks, d, ncls, init_seed, epochs, lr, hidden):
    ...
    given at test time. This is what PRISM matches WITHOUT being told the task id (it infers it
...
def param_count_prism(d, h, ncls, M):
...
def E1_main(n_seeds=10, K=5, d=24, ncls=8, epochs=15):
    ...
    def prism_run(tasks, s, **kw):
        p = PRISM(d=d, h=48, K=ncls, n_experts=K + 3, seed=s, **kw)
    ...
    order = ["backprop", "EWC", "replay(boundaries)", "oracle_multihead",
             "PRISM(DFA,no W^T)", "PRISM_exactW^T", "PRISM_noRoute(ablation)"]
    ...
    for s in range(n_seeds):
        ...
        for name, fn in [
            ...
            ("PRISM(DFA,no W^T)", lambda: prism_run(tasks, s, consolidate=True, route=True, feedback="random")),
            ("PRISM_exactW^T", lambda: prism_run(tasks, s, consolidate=True, route=True, feedback="exact")),
            ("PRISM_noRoute(ablation)", lambda: prism_run(tasks, s, route=False)),
        ]:
            R = fn()
    ...
    pc_prism = param_count_prism(d, 48, ncls, K + 3)
    print(f"  [params] backprop/EWC={pc_mlp}  PRISM(trainable)={pc_prism}")
    return {"ewc_lambda": best_lam, "rows": rows, "params": {"mlp": pc_mlp, "prism": pc_prism},
            "n_seeds": n_seeds, "K": K}
...
def E2_separability(n_seeds=5, K=5, epochs=15):
    ...
    for noise in [0.0, 0.3, 0.6, 0.9, 1.2]:
        ...
        for s in range(n_seeds):
            ...
            Rp, p = run_prism(tasks, 24, 8, s, epochs, K + 3, 48, consolidate=True)
        ...
        out.append({"noise": noise, "PRISM_FGT": pfm, "PRISM_ACC": pam,
                    "backprop_FGT": bfm, "backprop_ACC": bam, "experts_committed": ncommit})
        print(f"  noise={noise:<4} PRISM: ACC={pam:.3f} FGT={pfm:.3f} (committed~{ncommit})"
...
def E3_capacity(n_seeds=5, K=5, epochs=15):
    ...
    for M in [3, 4, 5, 6, 8]:
        ...
        for s in range(n_seeds):
            tasks = make_sperm(K, s)
            R, p = run_prism(tasks, 24, 8, s, epochs, M, 48, consolidate=True)
...
def E4_locality(n_seeds=5, K=5, epochs=15):
    print("\n" + "=" * 70 + "\nE4  LOCALITY: PRISM with RANDOM feedback (DFA, no W^T) vs exact (P2)\n" + "=" * 70)
    out = {}
    for fb in ["exact", "random"]:
        fa, aa = [], []
        for s in range(n_seeds):
            tasks = make_sperm(K, s)
            R, _ = run_prism(tasks, 24, 8, s, epochs, K + 3, 48, consolidate=True, feedback=fb)
            fa.append(R.forgetting()); aa.append(R.acc())
        fm, _ = ci95(fa); am, _ = ci95(aa)
...
def E5_impossible(n_seeds=5, K=3, epochs=120):
    ...
    for s in range(n_seeds):
        ...
        # PRISM on raw 2D input (its recognizer cannot separate shared-input domains)
        class T:  # lightweight task holder
            pass
        tasks = []
        for (tr, te) in tasks_raw:
            o = T(); o.Xtr, o.ytr = tr; o.Xte, o.yte = te; o.n_classes = 2; tasks.append(o)
        Rp, _ = run_prism(tasks, 2, 2, s, 30, K + 2, 32, consolidate=True, z_novel=5.0)
    ...
    print(f"  PRISM final ACC = {pam:.3f}   backprop final ACC = {bam:.3f}")
    print(f"  => PRISM does NOT exceed the ceiling: honest boundary confirmed "
          f"({'OK' if pam <= ceiling + 0.03 else 'VIOLATION'})")
    return {"ceiling": ceiling, "PRISM_ACC": pam, "backprop_ACC": bam}
