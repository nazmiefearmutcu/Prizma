"""Produce the headline figure for Prizma: retention curves + forgetting + separability."""
import sys, os, json
...
def retention(kind):
    ...
    if kind == "Prizma":
        p = Prizma(d=24, h=48, K=8, n_experts=K + 3, seed=0)
...
Rp, Rb = retention("Prizma"), retention("backprop")
...
for j, c in zip(range(3), ["#e63946", "#457b9d", "#2a9d8f"]):
    ...
    ax[0].plot(stages[j:], Rp[j:, j], "o-", color=c, lw=2.4, label=f"Prizma T{j}")
ax[0].set_title("A. Per-task retention through training\n(solid=Prizma, dashed=backprop)")
...
names = ["backprop", "EWC", "replay(boundaries)", "Prizma(DFA,no W^T)", "PRIZMA_noRoute(ablation)"]
...
ax[2].plot(noise, [r["PRIZMA_ACC"] for r in sw], "o-", color="#2a9d8f", lw=2.2, label="Prizma ACC")
ax[2].plot(noise, [r["backprop_ACC"] for r in sw], "o--", color="#9aa0a6", lw=1.8, label="backprop ACC")
ax[2].plot(noise, [r["PRIZMA_FGT"] for r in sw], "s-", color="#e63946", lw=2.2, label="Prizma FGT")
