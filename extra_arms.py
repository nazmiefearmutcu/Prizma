"""Committee-completing follow-up arms, written into the SAME $PRISM_RESULTS/gpu_bench.json
...
Two arms, both at D=128 MixedMQAR, gen-warm, per-model plateau, 5 seeds:
  ...
  ARM B  "FLOP-matched TF" — the headline (P2) is param-matched at d128L4H4, but the measured
         ledger shows PRISM-quad2-256 costs ~2.14x the param-matched TF's forward FLOPs/token.
         A deeper TF d128L9H4 matches PRISM-quad2's as-coded FLOPs to ~2% (4576 vs 4505
         kFLOP/tok; flop_ledger.py) AND has MORE params than PRISM -> deliberately generous to
         attention. Answers "is PRISM's recall just spent FLOPs?".

  ARM A  "parameter-efficiency / sharp contrast" — at the TINY matched scale d64L2H2 (130K),
         does PRISM-quad2 solve D=128 where the param-matched Transformer cannot? P1 already
         shows TF d64L2H2 = 0/3 (0.016). This adds PRISM-quad2 + PRISM-none x5 at 130K (and 2
...
Run on the Colab A100 (gist must include this file):
    PRISM_RESULTS=/content/drive/MyDrive/prism_results python -u extra_arms.py
"""
from gpu_bench import (run_cell, solve_stats, tf_factory, ps_factory, _load, _save, OUT, DEV)
from seq.tasks import MixedMQAR

V = 512
S5 = (0, 1, 2)   # 3 seeds (committee min ≥3): PRISM is ~30-57min/run on A100 — 5 seeds is intractable here
...
def main():
    ...
    # --- ARM B: FLOP-matched (deeper) Transformer d128L9H4 ~= PRISM-quad2-256 as-coded FLOPs ---
    ...
    q = [run_cell(res, f"p2eff.PRISM-quad2-d64L2H2.s{s}",
                  ps_factory(64, 2, 2, feat_map="quad2", feat_n2=224), TASK, 80000, s) for s in S5]
    summary["PRISM-quad2-d64L2H2"] = solve_stats(q)
    print(f"  -> PRISM-quad2: {summary['PRISM-quad2-d64L2H2']}", flush=True)

    n = [run_cell(res, f"p2eff.PRISM-none-d64L2H2.s{s}", ps_factory(64, 2, 2), TASK, 80000, s)
         for s in S5]
    summary["PRISM-none-d64L2H2"] = solve_stats(n)
    print(f"  -> PRISM-none:  {summary['PRISM-none-d64L2H2']}", flush=True)
    ...
    rl = [run_cell(res, f"p2eff.PRISM-randlin-d64L2H2.s{s}",
                   ps_factory(64, 2, 2, feat_map="rand_linear", feat_n2=224), TASK, 80000, s) for s in S5]
    summary["PRISM-randlin-d64L2H2"] = solve_stats(rl)
    print(f"  -> PRISM-rand_linear (control): {summary['PRISM-randlin-d64L2H2']}", flush=True)
