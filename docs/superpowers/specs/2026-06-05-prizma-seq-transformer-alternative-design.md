# Prizma-Seq → Transformer Alternative — Design Spec (2026-06-05)

> Goal (owner, approved scope = **"bar + aggressive frontier"**): develop Prizma-Seq at full speed
...
- **MQAR D=128 ✓** (1 of the 4 diagnostic legs): at matched params Prizma-quad2 ≈ tuned TF
...
2. **Induction** (in-context `…[A][B]…[A]→[B]`): Prizma-quad2 **≥0.98**; holds across prefix length
...
6. **Honesty controls** — param/FLOP-matched audited; ≥3 seeds + CIs (or seed ranges at n=3); causal
   ablation that the Prizma mechanism (not "a bigger RNN") is responsible; explicit failure/limits.

**Verdict rule:** Prizma-Seq earns "credible alternative in tested regime" iff legs 1–4 PASS at matched
...
- **Phase A — finish the diagnostic suite (the gap).** GPU runner `gpu_diag.py` (analogous to the
  verified `gpu_bench.py`): Induction + SelectiveCopy, Prizma-quad2 vs Prizma-none(ablation) vs tuned TF,
...
- **Phase B — char-LM leg.** GPU runner `gpu_charlm.py` (wraps `seq/charlm.py`): tiny-shakespeare
  char-LM, param-matched Prizma-quad2 vs TF, report test BPC + the margin, ≥2 seeds. Frontier: text8
...
- **Phase C — strengthen structural advantage.** (1) `flop_match_width.py` d208 control (already in
  ...
  so the TF's O(t) per-step term dominates and Prizma's O(1)-per-step yields a real wall-clock win
...
- **Phase E — final adversarial referee gate + report.** Update `docs/PRIZMA_SEQ_REPORT.md` (B1–B6
...
- **GPU runs:** Colab A100 via Claude-for-Chrome (secret-gist bootstrap, the verified pattern). Stream
  crash-safe JSON to Drive; resumable. Prizma ~30–57 min/run on A100 (sequential chunked-delta) → use 3
...
Param/FLOP-match every head-to-head (grow TF for any paid Prizma arm); measured FLOP+memory ledger
...
- A new attention PRIMITIVE (Prizma-Seq is a synthesis + the quad2 lever + framing).
