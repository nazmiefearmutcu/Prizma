# Prizma-Seq → Transformer-Alternative — SESSION HANDOFF
...
- **Goal (locked by owner):** push Prizma-Seq to be a *credible Transformer alternative*, proven on the
...
- **Seed-0 directional results (matched params, mixed-D training, fair protocol):**
  - **D=64:** Transformer **1.000**, Prizma-quad2 **1.000**, Prizma-none 0.931 → *parity*.
  - **D=128:** tiny matched TF **0.016** (can't learn at this scale), Prizma-quad2-256 **0.999**,
    Prizma-none 0.369. + flip-test confirms tiny TF stays flat even at constant high LR / 120k steps.
- **Committee R2 (adversarial) verdict = `NEEDS-EXPERIMENTS-FIRST`.** The D=128 claim is NOT yet
  ...
  (2) **FLOP gap undisclosed** — Prizma-quad2-256 = **2.1–2.7× TF FLOPs/token** (rectangular state =
...
- **NEXT: run `gpu_bench.py` on a CUDA GPU (Colab)** — it does all the rigor (scale search, ≥5-seed
  ...
  latency/memory). The notebook `PRIZMA_D128_GPU.ipynb` is built & ready. Then update the report and run
...
- **Local (Mac):** use system Python `/Library/Frameworks/Python.framework/Versions/3.13/bin/python3.13`
  ...
  `PYTHONPATH=/Users/nazmi/Desktop/Projeler/proje/Prizma`. Prizma is ~4–5× slower than the TF per step on MPS.
...
## 3. Architecture (`seq/`)
- **`prizma_seq.py`** — Prizma-Seq = Gated-DeltaNet-family mixer: per-head carried state `S` (precision-gated
...
2. **Per-model optimizer tuning** (LR *and* warmup): the TF needs **gen-warm** (lr=1e-3, warmup=2000);
   short "punchy" warmup destabilizes it (loss oscillates 0.8↔4.5). Prizma is robust to both. "Identical
...
- `parity_d64.json`: D=64 gen-warm — Transformer 1.000, **Prizma-quad2 0.99951**, Prizma-none 0.931.
- `d128_sweep.json`: D=128 gen-warm — Transformer **0.016**, **Prizma-quad2-256 0.99947**, Prizma-none 0.369.
...
- `flop_ledger.py` output: Prizma-quad2-256 / TF forward-FLOP ratio = **2.14× (ideal) – 2.67× (as-coded)**;
...
## 7. GPU plan — `gpu_bench.py` (CUDA) + `PRIZMA_D128_GPU.ipynb`
`gpu_bench.py` phases (resumable; writes `$PRIZMA_RESULTS/gpu_bench.json` incrementally):
...
`PRIZMA_D128_GPU.ipynb` (at repo root, 91KB, 19 cells, validated) self-contains everything: writes the
verified `seq/*` + `gpu_bench.py` + `flop_ledger.py` via `%%writefile`, mounts Drive, runs kernel
self-tests, the FLOP ledger, then all phases; saves to `Drive/MyDrive/prizma_results/gpu_bench.json` and
...
`/Users/nazmi/Desktop/Projeler/proje/Prizma/PRIZMA_D128_GPU.ipynb` → Runtime=A100/L4 → Run all → approve
...
- Poll Drive: `Google_Drive search_files` query `title contains 'gpu_bench' ` (or list the prizma_results
...
2. **Adjudicate the flip-test from Phase 1:** does a bigger TF SOLVE D=128? If yes → honest framing =
   "matched-tiny TF under-capacity; Prizma-quad2 solves at matched params + O(1) + far cheaper than scaling
...
4. **Update `docs/PRIZMA_SEQ_REPORT.md`** with the real numbers (it still has `_pending_` rows), the FLOP
...
Param-match every head-to-head (grow the TF for any paid Prizma arm); ship a measured FLOP+throughput
...
checkpoints. Memory file: `~/.claude/projects/-Users-nazmi/memory/prizma-seq-transformer-alternative.md`.
