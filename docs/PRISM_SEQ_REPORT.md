# PRISM-Seq — Is It a Credible Transformer Alternative? (Living Report)
...
> *PRISM-Seq is a credible efficient-attention-replacement candidate at small scale, in the tested
...
**PRISM-Seq clears the project's pre-registered §4 falsifiable bar in the tested regime** (small scale,
...
**overtakes attention only at long context (n≥32k; PRISM is ~1.5× slower below that)**; the named mechanism
is causally responsible; and on length-extrapolation PRISM **degrades far more gracefully** than a RoPE
Transformer (a *relative* win — PRISM's own absolute accuracy still falls). It is a **candidate**, not a
...
| B2 Induction | **PASS** | quad2 0.9995 (3/3) vs TF 0.996 _(non-discriminating: PRISM-none also solves)_ |
| B3 Selective-copy | **PASS** | selective 0.9991; fixed-control spread 1e-4 _(also non-discriminating)_ |
| B4 Char-LM (text8) | **PASS (within margin; does NOT beat TF)** | PRISM 1.7496 vs TF 1.7254 → +0.024 *worse*, under the +0.05 bar |
| B5 Inference | **PASS (memory)** | constant 17.9MB ∀n → 28–455× less mem (analytic+measured); O(1)/step. Latency crossover **only at n≥32k** (2.4× @65k); PRISM ~1.5× slower below |
| B6 Causal | **PASS** | quad2 ≫ rand_linear ≈ none ≫ TF (the monomials, not "a bigger RNN") |
| Length-extrap (frontier) | **WIN (relative)** | PRISM 10× better retention than RoPE-TF @8× — but PRISM's own absolute acc is only ~0.40 @8× |

**Honest limits (binding, not buried):** (1) on char-LM PRISM is competitive *within* the +0.05 margin —
it does **NOT beat** the TF (1.7496 vs 1.7254). (2) PRISM is **~5× slower to _train_** per step on GPU
...
**On the field-standard MQAR D=128 rung, PRISM-Seq's parameter-free quadratic rectangular-delta-state
...
1. **Descriptive parity at matched params.** At ~860K matched params (d128L4H4), 3 seeds, all SOLVE
   (3/3): **PRISM-quad2 seeds [0.995, 0.998, 0.999] (median 0.998) vs tuned Transformer [0.979, 0.9995,
   ...
   seed ranges, not CIs). PRISM-quad2 additionally **converges in ~⅓ the steps** (ignites 16–20k vs TF
...
2. **≥3.5× parameter-efficiency (existence result on a coarse grid).** At 130K matched params (d64L2H2):
   **PRISM-quad2 solves D=128 3/3 (seeds [0.999, 0.997, 0.935]) where the param-matched Transformer fails
...
3. **Structural advantage — MEMORY (analytically exact), NOT speed.** Inference state size is
   **analytically counted** (closed-form float counts, not a runtime allocation measurement): PRISM
   carried state = **147,456 floats ∀ sequence length**; the TF KV-cache grows linearly → **28.4× larger
   at n=4096** (crosses PRISM at n≈144). Decode **latency IS measured** (A100): per-step is flat for
   **both** models at n≤4096 (PRISM ~7.0ms, TF ~4.5ms) — i.e. **the experiment does NOT observe the
   O(t)-vs-O(1) latency separation** (both overhead-bound); the asymptotic crossover is an *analytic
   expectation*, not a measured result, and **wall-clock latency does NOT favor PRISM here** (PRISM ~1.55×
   slower/step). **Net at this scale: +114% FLOPs/token, +55% per-step latency, −96% memory @n=4096 → the
   advantage is memory, not speed or compute.** FLOP cost disclosed: PRISM-quad2 = **2.14× the TF's
   forward FLOPs/token**.
...
> PRISM, which prefers 2e-3) was used on **B1/B1b/B2/B3/B6**; only **B4** swept a per-model LR — see the
...
param-matched parity already shows PRISM ≈ TF at EQUAL params despite 2.14× FLOPs, so "is it just
...
## What PRISM-Seq is (one paragraph)
...
PRISM's per-token free energy `F_t(S)=½‖v_t − S k_t‖²`. Reads are recognition-by-reconstruction
...
1. **The write gate `β_t = σ(W_β x_t)` is input-dependent** (so the chunk-parallel training form is
   ...
   `u_t = β_t(v_t − S_{t-1}k_t) = β_t ε_t` (PRISM's `dW ∝ (Πε)⊗r`).
2. **A short causal depthwise conv (kernel 4) precedes the projections** — the standard component in
   ...
   lr=1e-3** (PRISM-Seq needs 2e-3 — a single-LR artifact, now controlled by running B6's ablations at
...
3. **Honest consequence:** at the parallelizable level PRISM-Seq's mixer **coincides with
   Gated-DeltaNet + short conv + a small window head** — all borrowed, known-good components. The
   architecture is a *synthesis*, not a new primitive. PRISM-Seq's distinct, separately-tested
...
4. **What "Transformer alternative" means here (and why it is still a real claim):** the DeltaNet/
   ...
   *something that can stand in for the Transformer* — is satisfied by a member of that family that
   ...
   a measured O(1)-state inference advantage. The *novelty* of PRISM-Seq within that family is the
...
each scale: d128L4H4 → **TF 857,216 vs PRISM-quad2 862,368** (+0.6%).

| Scale | TF kFLOP/tok | PRISM-quad2-256 as-coded | PRISM ideal (banded window) | Ratio as-coded / ideal |
|---|---|---|---|---|
...
PRISM's fixed d_h·d_φ delta-state does not. **FLOP-matched TF arm** (Phase 2b): a deeper
**TF d128L9H4** (4575.5 kFLOP/tok) matches PRISM-quad2's as-coded FLOPs to ~2% — and carries *more*
params than PRISM, so it is deliberately generous to attention.
...
- **P1 (TF solving-scale {d64L2H2, d128L2H4, d128L4H4, d256L4H8} × 3 seeds):** if a bigger TF
  ...
  cannot" — honest framing becomes "PRISM-quad2 solves at matched params + O(1); attention needs to
  scale". If even d256L4H8 fails, investigate task realism before any claim.
- **P2 (headline, d128L4H4, ≥5 seeds):** TF vs PRISM-none vs PRISM-quad2 → solve-rate + median + CI.
- **P2b (FLOP-matched TF d128L9H4, ≥5 seeds):** is PRISM's win just spent FLOPs?
...
igniting by step 24k/32k/28k**. The param-matched-to-tiny-PRISM TF (**d64L2H2, 130K: 0/3, 0.016**
all seeds, full 80k cap) does **not** — so the tiny-TF failure is **under-capacity at that size**,
NOT "attention cannot recall". Honest consequence: PRISM-quad2 reaches D=128 recall at **130K**
...
SOLVE D=128 (3/3). **PRISM-quad2 seeds [0.995, 0.998, 0.999] (median 0.998) vs Transformer seeds
...
not t and runs above 1.0 at ceiling, so we report seed ranges, not CIs). At matched params, PRISM-quad2
> PRISM-none (median 0.958) → the rectangular-delta-state lever helps even at this scale, and **converges
fastest of all three** (ignites by step 16–20k vs PRISM-none 42–60k vs TF 44–80k — ~⅓ the Transformer's
...
| P2 headline (matched d128L4H4, 3 seeds) | TF **3/3** (med 0.9995); PRISM-none **3/3** (0.958); PRISM-quad2 **3/3** (0.9983) — **parity, quad2≈TF, fastest ignition 16-20k** | **headline ✓** |
...
| P2eff parameter-efficiency + causal @ d64L2H2 (130K) | **PRISM-quad2 3/3 (median 0.997)** SOLVES D=128 @130K where the param-matched **TF is 0/3 (0.016)** → ~3.5× param-efficiency vs the smallest clean TF solver (d128L2H4, 461K). **Causal (guardrail #5, 3 seeds): quad2 3/3 (0.997) ≫ rand_linear 0/3 (med 0.589) ≈ none 0/3 (med 0.518) ≫ TF 0/3 (0.016)** → only quad2 solves; the gain is the QUADRATIC monomials, NOT any d_φ=256 expansion (rand_linear = same-dim random *linear* map, rank≤d_h = no gain over none). | **param-efficiency ✓ + causal ✓** |
| P3 D-frontier | **deferred** (secondary capacity curve; PRISM ~50min/run on A100 made the full 45-cell sweep intractable — noted as future work) | _deferred_ |
| P4 ablation @ d128L4H4 | **moved to d64L2H2** (at d128L4H4 PRISM-none already solves 0.958 ≈ quad2, so the causal contrast is uninformative there; the sharp contrast + rand_linear control is run at d64 in P2eff) | _moved→P2eff_ |
| P5 O(1): memory (ANALYTIC) + latency (measured) | **memory (analytically counted, n-independent formula): PRISM state CONSTANT 147,456 floats ∀n; TF KV-cache linear (131K@128 → 4.19M@4096) → TF uses 28.4× more @n=4096, crosses PRISM @n≈144. latency (MEASURED wall-clock, A100): both per-steps are flat/overhead-bound — PRISM ~7.0ms ∀n (O(1)), TF ~4.5ms ∀n; the run does NOT observe the O(t)-vs-O(1) separation, PRISM is ~1.55× slower/step, crossover is an analytic expectation not measured **at n≤4096**. (The v3 B5 frontier later MEASURED it directly out to n=65536 → crossover @ n=32768; see B5 + frontier.)** | **mem ✓ (analytic); latency crossover MEASURED at n≥32k in the B5 frontier (NOT at n≤4096)** |

---
## Results vs the bar (filled as experiments complete)

| # | Item | Pass condition | PRISM-Seq | Transformer | Verdict |
|---|---|---|---|---|---|
...
| B2 | Induction | ≥0.98; 64–256 gap ≥0.95 | **3/3, median 0.9995** (256: 0.9995) ✓ ignites ~18k | 3/3>0.9 median 0.996 (s2=0.903 weak) | **PASS** (param-matched d128L2H4 0.64%; single lr=1e-3; `gpu_diag.json`). _Non-discriminating: PRISM-none also solves (median 1.0); the lever's edge is B1b/B6, not here._ |
| B3 | Selective copy | selective ≥0.97, ≥T−0.02; control gate | **selective 3/3, median 0.9991**; fixed-control 0.9999 | selective 3/3, median 0.9994; fixed-control 1.0 | **PASS** (param-matched d128L2H4 0.64%; quad2≥T−0.02, fixed-control spread 1e‑4; `gpu_diag.json`). _Non-discriminating: PRISM-none also 0.9994 — selcopy/induction are solved by PRISM regardless of feat_map; the lever's edge shows in B1b/B6, not here._ |
| B4 | Char-LM | test BPC ≤ T+0.05 | **text8 best_bpc 1.7496** (2 seeds [1.751, 1.749], val-selected, NO overfit) | **1.7254** (val-selected, no overfit) | **PASS** (margin TF−PRISM = **−0.024 < +0.05**; `gpu_charlm2.json`: text8-10M, AdamW wd=0.1, eval@250, val-based early-stop, param-match −0.13%, n=2). _Honest: PRISM is competitive **within** the margin, NOT beating TF; ~5× slower to train (1889 vs 361 s/arm). An earlier shakespeare run OVERFIT (PRISM best 2.30 vs TF 2.21 = −0.09 fail; raw not retained on disk — superseded) and is replaced by this credible text8 result. text8 split is contiguous (train 0–10M / val 10–10.5M / test 10.5–11M) with **no guard band** → <0.05% boundary leak, symmetric across both arms._ |
| B5 | Inference advantage | flat latency + constant mem | **peak mem 17.9MB ∀n (measured, constant); per-step FLAT ~7.0ms ∀n (O(1))** | peak 49→**562MB**; per-step 4.5→**16.9ms** (n4k→64k) | **PASS — primary edge is MEMORY** (constant state → 28–455× less; analytic + measured). _Secondary:_ measured latency crossover @ **n=32768** (`gpu_latency.json`, A100, n→65536, both sizes): @65536 PRISM **2.4× faster small** (7.1 vs 16.9ms) / **2.8× big** (13.9 vs 38.9ms, measured). **Below n≤16k PRISM is ~1.3–1.5× SLOWER** (overhead-bound) → the latency win is long-context-only (disclosed). Latency = single A100 run, median of 5 reps (2 warmup), no seed-CI. |
...
| Axis | Result | Verdict |
|---|---|---|
| **Length-extrapolation** (induction, train L=64, eval 64→512; `gpu_lengen.json`, 3 seeds) | retention@8× (512/64): **PRISM-quad2 0.398 vs TF(RoPE) 0.041** = 10×. @2× (128): PRISM **0.966** vs TF **0.485**; @4× (256): 0.720 vs 0.087 | **PRISM ≫ TF (RELATIVE win)** — the position-free delta path degrades far more gracefully than RoPE attention (which collapses to ~chance by 2×). **Honest: this is a *relative* result — PRISM's own absolute accuracy at 8× is only ~0.40** (much better than TF's 0.04, but not "good" in absolute terms). |
| **Width FLOP-match** TF d208L4H4 (2.18M, ~2.5× the param-matched count; `gpu_bench.json` p2c) | **0/3, median 0.0221** (s0/s1/s2 = 0.022, CI ±0.0002 — deterministic, not seed noise) | **Inconclusive — NOT a PRISM win (optimization-confounded).** Tell-tale: the *same-depth but narrower* d128L4H4 (857K) solves MQAR 2–3/3, yet the wider d208L4H4 fails identically across all 3 seeds → this is **consistent with** an LR-transfer artifact (optimal LR scales ~1/width per μP; recipe tuned at d128) rather than capacity — but that cause is **inferred, not separately confirmed**; the decisive check (a per-width LR re-sweep at d208) was not run. Both FLOP arms (depth d128L9H4 1/3, width d208L4H4 0/3) are therefore excluded from the verdict; clean evidence stays param-matched parity + ≥3.5× param-efficiency, never a per-FLOP claim. |
| **O(1) latency crossover** n∈{4k..65k}, two model sizes (`gpu_latency.json`) | small: crossover **n=32768**; @65536 TF 16.9ms vs PRISM **7.1ms** (2.4×), peak 562MB vs **17.9MB** (31×). big: crossover ~32k; @65536 TF 38.9ms vs PRISM ~14ms (2.8×), 4532MB vs **232MB** (19×) | **PRISM O(1) confirmed** — per-step FLAT across a 16× span of n; TF grows O(t). Real wall-clock win emerges at **n≥32k** (below that PRISM is overhead-bound, ~1.5× slower — disclosed, not hidden). |
| **Credible char-LM** text8-10M + weight_decay + val-selected TEST BPC (`gpu_charlm2.json`) | TF **1.7254** vs PRISM-quad2 **1.7496** (2 seeds, no overfit, vs random 4.755); margin **−0.024** | **PASS (= B4)** — the overfitting-robust char-LM settles B4 in PRISM's favor (within +0.05) where the shakespeare overfit recipe had failed it. |

### Pre-registration (committed BEFORE seeing B1b multi-seed results — methodological honesty)
- **D\* prediction:** PRISM-Seq's recall state is `H=2` heads × rank `d_h=32` ≈ up to ~64 near-
  orthogonal bindings in principle, but recall degrades before saturation. **Pre-registered D\* = 32**
  for the matched config (d=64, d_h=32): PRISM-Seq expected within 0.03 of the Transformer up to
...
- **Validated build decisions (from systematic isolation, 1-seed):** (1) PRISM-Seq solves AR — tiny
...
- **LR protocol (corrected disclosure — referee-flagged).** Only the char-LM **B4** actually sweeps a
  ...
  per-model sweep. This is **conservative for PRISM** (the report notes PRISM prefers 2e-3, so a shared
  1e-3 disadvantages PRISM, not the TF), so it does not inflate PRISM's results — but the earlier
...
- **Eval.** ≥~1–2k freshly-generated sequences per eval (synthetic tasks draw i.i.d., so eval is
  ...
  float counts in `gpu_bench.py` phase5: TF KV = 2·L·H·d_h·n; PRISM state = L·H·d_h·(d_h+d_φ) + window
...
- MQAR rung1, matched params (~100K), identical budget (2500 steps): PRISM-Seq **0.871** vs
...
| **Precision/surprise signal used causally for gating** | PRISM | new (tested in B6) |
| **Task-free continual SEQUENCE modeling** | PRISM | new axis (secondary) |
