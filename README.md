...
|---|---|---|
| **PRISM-Seq** | Can a parameter-free quadratic delta-state sequence mixer stand in for attention at small scale? | **Candidate** — clears the §4 diagnostic bar param-matched vs a tuned Transformer; constant-memory + long-context O(1)-latency edge; honest losses disclosed. |
| **PRISM** | Can a backprop-free, fully-local learner do task-boundary-free continual learning? | **Zero forgetting** in the input-distinguishable regime, beating backprop & EWC — no replay, no boundaries, no weight transport. |

---

## PRISM-Seq — efficient-attention-replacement candidate

PRISM-Seq is a Gated-DeltaNet-family sequence mixer whose novel lever is a **parameter-free
...
| Char-LM (text8) | **PASS** | PRISM 1.7496 vs TF 1.7254 BPC — within the pre-registered +0.05 bar (**does not beat** TF) |
...
latency win is long-context-only (PRISM is ~1.3–1.5× *slower* below n≈16k) and PRISM trains ~5×
...
- Full writeup + adversarial referee trail → **[`docs/PRISM_SEQ_REPORT.md`](docs/PRISM_SEQ_REPORT.md)**
- Raw A100 results (auditable) → `results/gpu_{bench,diag,lengen,latency,charlm2}.json` + `results/v3_campaign_results.md`
- Code → `seq/` (mixer, tasks, transformer baseline), `gpu_*.py` (GPU runners), `PRISM_run_*.ipynb` (Colab bootstrap)

```bash
# local kernel self-tests / smoke (CPU/MPS), then the GPU runners on an A100:
PRISM_RESULTS=results python gpu_diag.py induction selcopy   # B2/B3
PRISM_RESULTS=results python gpu_charlm2.py --skip_none      # B4 (text8)
PRISM_RESULTS=results python gpu_latency.py                  # B5 latency/memory
PRISM_RESULTS=results python gpu_lengen.py                   # length-extrapolation
```

---

## PRISM — backprop-free, fully-local continual learning

**Precision-Routed Inference with Synaptic Metaplasticity** — a backprop-free, fully-local,
predictive-coding learning architecture targeting neuromorphic/analog hardware.

PRISM demonstrates **task-boundary-free, task-label-free continual learning**: in an
...
| **PRISM (DFA, no W^T)** | **0.834** | **0.000** | **none** | **none** | **none** |
| PRISM (exact W^T) | 0.708 | 0.000 | none | none | yes |
| PRISM_noRoute *(ablation)* | 0.446 | 0.489 | — | — | — |

PRISM sits **between replay and the task-id-oracle**, matching the oracle's zero forgetting
...
- Full writeup (equations, borrowed-vs-new ledger, neuromorphic mapping, limits) → **[`docs/PRISM.md`](docs/PRISM.md)**
- Code → `src/` (prism + baselines + data + metrics), `experiments/` (E1–E5 suite + figure)
